...
class Event():
    ...
    def add_award_init(self, awa: str, haswinner: bool, presenters: Optional[set[int]] = None, typ: Optional[WinType] = None, nominees: Optional[Union[set[int], set[str]]] = None, winner: Optional[Union[int, str]] = None):
        if(not (awa in self.awards)):
            ...
            if(typ == WinType.PERSON):
                if(nominees != None):
                    ...
                    if(winner != None and (not (winner in nominees))):
                        nominees.append(self.name_to_id[winner])
                
                if(haswinner and winner != None):
                    winner = self.name_to_id[winner]
                    
            elif(nominees != {}):
                nominees = list(x for x in nominees)
                if(winner != None and (not (winner in nominees))):
    ...
    def __str__(self):
        ...
        for x in self.awards:
            wnr = ""
            tempwin = self.awards[x].winner
            if(tempwin != None):
                if(self.awards[x].nomineeType == WinType.PERSON):
                    wnr = str(self.people[tempwin])
                else:
                    wnr = str(tempwin)
...
def input_parameters(params):
    ...
    events = {
        ...
        'event_nul': Event("Golden Globes 2013") # no data
    }
    ...
    for i in list(params["award_data"].keys()):
        ...
        events['event_key'].add_award_init(i, True, params["award_data"][i]["presenters"], wt, params["award_data"][i]["nominees"], params["award_data"][i]["winner"])
        
        events['event_nom'].add_award_init(i, False, params["award_data"][i]["presenters"], wt, params["award_data"][i]["nominees"], params["award_data"][i]["winner"])

        events['event_pre'].add_award_init(i, False, params["award_data"][i]["presenters"])

        events['event_awa'].add_award_init(i, False)
